...
Por otra parte, rechazamos la hipótesis de que el modelo bifactor reproduce la misma matriz Σ que el saturado, por lo que concluimos que nuestro modelo no ajusta a los datos, y que por lo tanto, no es apropiado.
\end{document}