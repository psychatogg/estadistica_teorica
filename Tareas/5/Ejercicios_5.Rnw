...
\\
& \text{Aplicamos propiedades del logaritmo para facilitar los cálculos posteriores} \\
& \ln l(\theta)=5 ln(\theta+1)+\theta \ln (0,15) \\
\\
& \text{Localizamos el máximo de dicha función (máxima verosimilitud)} \\
& \frac{d}{d \theta} \ln l(\theta)=5 \cdot \frac{1}{\theta+1}-1,9 \\
& \frac{5}{\theta+1}-1,9=0 \\
& \frac{1}{\theta+1}=0,38 \\
& \theta+1=2,63 \\
& \theta=1,63 \\
\\
& \frac{d^2}{d \theta} \ln (\theta)=5 \cdot(-1) \cdot(\theta-1)^{-2}= \\
